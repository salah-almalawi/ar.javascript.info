...
يُعد التابع `arr.splice` بمثابة "سكين الجيش السويسري" للمصفوفات، إذ يمكنه فعل أيّ شيء: إدراج العناصر، حذفها، واستبدالها.
...
يُعدّل هذا التابِع المصفوفة `arr` بدءًا من الفهرس `start`: فيحذف عدد `deleteCount` من العناصر ويُدرج مكانها العناصر `elem1, ..., elemN`. كما ويُعيد مصفوفةً فيها العناصر المحذوفة.
...
arr.splice(1, 1); // من الفهرس 1، احذف عنصراً واحداً
...
// أزِل العناصر الثلاثة الأولى وعوّضها بتلك الأخرى
...
يمكننا هنا وفي توابِع المصفوفات الأخرى استعمال الفهارس السالبة. وظيفتها تحديد المكان بدءًا من نهاية المصفوفة، هكذا:

```javascript run
let arr = [1, 2, 5];

// بدءاً من الفهرس -1 (خطوة واحدة من النهاية)
// لا تحذف شيئاً
// ثم أدرج 3 و 4
arr.splice(-1, 0, 3, 4);
...
هذا التابِع يشبه تابِع السلاسل النصية `str.slice`، ولكن بدل السلاسل النصية الفرعية، يُعيد المصفوفات الفرعية.
...
يمكننا أيضًا استدعاؤها بلا وُسطاء: يُنشئ `arr.slice()` نسخة عن `arr`. نستعمل هذا غالبًا لأخذ نسخة وإجراء تعديلات عليها دون تعديل المصفوفة الأصلية، وتركها كما هي.
...
وهو يقبل أيّ عدد من الوُسطاء، أكانت مصفوفات أو قيم. أمّا ناتجه هو مصفوفة جديدة تحوي العناصر من `arr`، ثم `arg1` فَـ `arg2` وهكذا دواليك. لو كان الوسيط `argN` نفسه مصفوفة، فستُنسخ كل عناصره، وإلّا فسيُنسخ الوسيط نفسه.
...
نرى في هذا المثال مصفوفة من المستخدمين، لكلّ مستخدم حقلان `id` و`name`. نريد الذي يتوافق مع الشرط `id == 1`:
...
function compare(a, b) {
  ...
  if (a == b) return 0; // لو تعادلت القيمتان
...
لنتوقف لحظة ونفكّر فيما يحدث تمامًا. أتتّفق بأنّ المصفوفة `arr` يمكن أن تحتوي أيّ شيء؟ أيّ شيء من الأعداد أو السلاسل النصية أو الكائنات أو غيرها. كلّ ما لدينا هو مجموعة من العناصر. لترتيبها نحتاج دالة ترتيب تعرف طريقة مقارنة عناصر المصفوفة. مبدئيًا، الترتيب يكون بالسلاسل النصية.

يُنفّذ التابِع `arr.sort(fn)` خوارزمية ترتيب عامّة. لا يهمّنا كيف تعمل داخليًا (غالبًا ما تكون "quicksort" أو "Timsort" مُحسّنة). هي تمشي على المصفوفة، وتُقارن عناصرها باستخدام الدالة المُقدّمة وتُعيد ترتيبهم، وكلّ ما نحتاجه هو توفير الدالة `fn` التي تُجري الموازنة.
...
تذكر الـ `strings` في خوارزميات الموازنة؟ إنها توازن المحارف عن طريق الكود الخاص بها.
...
لو ناديت `split(s)` وتركت `s` فارغًا فستُقسم السلسلة النصية إلى مصفوفة من الأحرف:
...
نفس الحال مع التابعين `arr.reduce` و `arr.reduceRight`، إلّا أنهما ليسا بالسهولة نفسها. يُستعمل هذان التابعان لحساب قيمة واحدة حسب عناصر المصفوفة.
...
* `accumulator`: هو نتيجة لكل الدوال السابقة ويعتبر مساوياً لـ `initial` في أول مرة.
* `item`: العنصر الحالي في المصفوفة.
* `index`: مكان العنصر (فهرسه).
* `array`: المصفوفة نفسها.
...
1. في أوّل مرّة، قيمة `sum` هي قيمة `initial` (آخر وسيط في reduce) وتساوي `0`، و `current` هي أوّل عنصر في المصفوفة وتساوي `1`. إذًا فناتج الدالة هو `1`.
...
…ولكن، المصفوفات تستعمل كثيرًا جدًا لدرجة تقديم تابِع خاص لهذا الغرض: `Array.isArray(value)`‎. يُعيد هذا التابِع `true` لو كانت `value` مصفوفة حقًا، و`false` لو لم تكن.
...
تقبل أغلب توابِع المصفوفات تقريبًا، التوابع التي تستدعي دوالًا (مثل `find` و`filter` و`map`، عدا `sort`) – تقبل المُعامل الاختياري `thisArg`.
...
يمكن استبدال استدعاء `users.filter(army.canJoin, army)` بالتعليمة التي تُؤدّي ذات الغرض `users.filter(user => army.canJoin(user))`. نستعمل الثانية أكثر من الأولى إذ أنّ الناس تفهمها أكثر.
...
* `slice(start, end)` – أنشِئ مصفوفة جديدة وانسخ عناصرها بدءًا من `start` وحتّى `end` (ولكن دون `end`).
* `concat(...items)` – أعِد مصفوفة جديدة: انسخ كل عناصر المصفوفة الحالية وأضَِف إليها العناصر `items`. لو كانت واحدة من عناصر `items` مصفوفة أيضًا، فستُنسخ عناصرها بدل المصفوفة نفسها.

**للبحث عن العناصر:**

* `indexOf/lastIndexOf(item, pos)` – ابحث عن العنصر `item` بدءًا من المكان `pos`، وأعد فهرسه لو وجدته وإلا فأعد `-1`.
* `includes(value)` – تُعيد `true` لو كانت المصفوفة تحوي القيمة `value`، وإلا فتعيد `false`.
* `find/filter(func)` – ترشيح العناصر عبر دالة، وتُعيد أول قيمة (في `find`) أو كل القيم (في `filter`) التي تعيد الدالة لها `true`.
* `findIndex` – مثل `find` ولكن تعيد الفهرس بدل القيمة.
...
لاحظ أنّ التوابِع `sort` و `reverse` و `splice` تُعدّل المصفوفة نفسها.
...
تتصرف هذه التوابع مثل المعاملات `||` و `&&`: لو أعادت `fn` قيمة حقيقية (truthy)، يُعيد `arr.some()` القيمة `true` حالاً ويتوقف عن المرور على باقي العناصر؛ ولو أعادت `fn` قيمة خاطئة (falsy)، يُعيد `arr.every()` القيمة `false` حالاً ويتوقف أيضًا.

يمكننا استخدام `every` لمقارنة المصفوفات:
...
* `arr.fill(value, start, end)` – يملأ المصفوفة بتكرار القيمة `value` من الفهرس `start` إلى `end`.
...
* `arr.flat(depth)` / `arr.flatMap(fn)` – يُنشئ مصفوفة مسطحة جديدة من مصفوفة متعددة الأبعاد.

للقائمة الكاملة، راجع الدليل (manual).

قد يبدو من النظرة الأولى أن هناك العديد من الطرق، ويصعب تذكرها. ولكن في الواقع هذا أسهل بكثير.
انظر من خلال ورقة الغش فقط لتكون على دراية بها. ثم حل مهام هذا الفصل للممارسة، بحيث يكون لديك خبرة في أساليب المصفوفات.
