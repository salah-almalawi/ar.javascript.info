...
* **أعداد عادية** تخزَّن بصيغة 64-بت IEEE-754، تُعرف أيضًا بـ "الأعداد العشرية مضاعفة الدقة" (double precision floating point numbers). هذا النوع هو ما سنستعلمه أغلب الوقت وسنسلط عليه الضوء في هذا الفصل.
* **أعداد صحيحة كبيرة** (BigInt numbers) تمثِّل عددًا صحيحًا متغير الحجم، إذ قد نلجأ إليها أحيانًا لأن النوع السابق لا يمكن أن يتجاوز القيمة  أو أن تقل عن ، وسنخصص لهذا النوع فصلًا خاصًا به نظرًا للحاجة إليه في حالات خاصة.
...
يمكننا أيضًا استخدام الشرطة السفلية `_` كفاصل:

```javascript
let billion = 1_000_000_000;
```

هنا تلعب الشرطة السفلية `_` دور **"السكر البرمجي" (syntactic sugar)**، فهي تجعل العدد أكثر وضوحًا للقراءة. ببساطة، يتجاهل محرك JavaScript الشرطة السفلية `_` بين الأرقام، لذا فهو تمامًا نفس المليار المذكور أعلاه.

في الواقع، نحن نحاول تجنب كتابة سلاسل طويلة من الأصفار؛ فنحن "كسالى" جدًا للقيام بذلك. لذا قد نحاول كتابة شيء مثل `"1bn"` للمليار أو `"7.3bn"` لـ 7 مليارات و300 مليون. وينطبق الشيء نفسه على معظم الأعداد الكبيرة.

في JavaScript، يمكننا اختصار العدد عن طريق إلحاق حرف `"e"` به وتحديد عدد الأصفار:

```javascript
alert( 7.3e9 ); // 7.3 بليون (تساوي 7300000000 أو 7_300_000_000)
let billion = 1e9;  // بليون، حرفيًا: 1 وجانبه 9 أصفار
```

بمعنى آخر، الحرف `e` يضرب العدد في `1` متبوعًا بعدد الأصفار المحدد.

```javascript
1e3 = 1 * 1000 // e3 تعني الضرب في 1000
1.23e6 = 1.23 * 1000000 // e6 تعني الضرب في 1000000
```
...
> [!CAUTION]
> إن وضعنا نقطة واحدة فقط `123456.toString(36)` فسيكون هناك خطأ، لأن JavaScript سَتعتبر أن النقطة هي فاصلة عشرية وأن ما بعدها هو جزء عشري للعدد. فإذا وضعنا نقطة أخرى فستعرف أن الجزء العشري فارغ وتنتقل إلى الدالة. يمكن كتابتها بهذه الطريقة أيضًا `(123456).toString(36)`.
...
* **`Math.floor`**: يقرب لأسفل: `3.1` تصبح `3` و `-1.1` تصبح `-2`.
* **`Math.ceil`**: يقرب لأعلى: `3.1` تصبح `4` و `-1.1` تصبح `-1`.
* **`Math.round`**: يقرب إلى أقرب عدد صحيح: `3.1` تصبح `3` و `3.6` تصبح `4`. وفي الحالة المتوسطة `3.5` يقرب لأعلى لتصبح `4` أيضًا.
* **`Math.trunc`** (غير مدعوم في متصفح Internet Explorer): يزيل أي شيء بعد الفاصلة العشرية دون تقريب: `3.1` تصبح `3` و `-1.1` تصبح `-1`.
...
تُغَطِّي هذه التوابع جميع الاحتمالات الممكنة للتعامل مع الجزء العشري للعدد، لكن ماذا إن كنا نريد تقريب العدد إلى خانة محدَّدة بعد الفاصلة العشرية؟

مثلًا، لدينا العدد `1.2345` ونريد تقريبه إلى خانتين لنحصل على `1.23` فقط. يوجد طريقتين للقيام بذلك:
...
**2- يقرب التابع [`toFixed(n)`](https://www.google.com/search?q=%5Bhttps://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/toFixed%5D(https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/toFixed)) العدد المستدعى معه إلى الخانة `n` بعد الفاصلة العشرية ويُرجِع تمثيلًا نصيًا للنتيجة.**
...
بمعنى آخر، ما هو `0.1`؟ هو واحد مقسوم على عشرة `1/10`، عُشر. ويكون من السهل تمثيله بنظام الأعداد العشري. موازنة بالثلث: `1/3`. الذي يصبح بكسور غير منتهية `0.33333(3)`.
...
وعند جمع عددين، فإن الأجزاء المفقودة تظهر. هذا يفسر لِمَ `0.1 + 0.2` لا تساوي `0.3` بالضبط.

> [!IMPORTANT]
> **ليس فقط JavaScript (Not only JavaScript)**
...
يمكننا أيضا ضرب الأعداد مؤقتًا في 100 أو أي عدد أكبر لتحويلها إلى أعداد صحيحة ثم إعادة قسمتها على العدد المضروب فيه. هكذا قد ترتفع نسبة الخطأ كما لو كنا نتعامل مع أعداد صحيحة لكننا سنتخلص منها بالقسمة:
...
لذا، يقلل نهج الضرب والقسمة الخطأ لكنه لا يزيله كليًا. قد نحاول تجنب الأجزاء كليًا في بعض الأحيان، كما لو كنا نتعامل مع متجر إلكتروني حتى نتمكن من تخزين الأسعار بالسنت بدلًا من الدولار. لكن ماذا إن وضعنا تخفيضًا بـ 30%؟ لنكن أكثر دقة، يكون تجنب الأجزاء كليًا نادرًا جدًا. قم بتقريبها فقط لتتخلص من الأجزاء غير المرغوبة عند الحاجة.
...
هذه الحالة تعاني من المشكلة ذاتها: ضياع الدقة. يوجد 64 بِت للعدد، يمكن استخدام 52 منها لتخزين العدد، لكنها غير كافية. لذا يظهر الرقم الأخير. لا تعرض JavaScript خطأ في مثل هذه الحالات فهي تقوم بأفضل ما لديها لملائمة العدد للصيغة المطلوبة. لكن، لسوء الحظ هذه الصيغة ليست كبيرة كفاية.

### صفران

نتيجة أخرى سلبية للتمثيل الداخلي للأعداد هو وجود صفرين `0` و `-0`. ذلك لأن الإشارة تُمَثَّل ببِت مستقل، لذا فيمكن لأي عدد أن يكون سالبًا أو موجبًا بما في ذلك الصفر. لا يكون هذا الفرق ملحوظًا في أغلب الحالات، لأن المعامِلات مُعَدَّة لتعاملهما كعدد واحد.
...
تنتمي هذه القيم إلى النوع `number`، لكنها ليست أعدادًا حقيقية، لذا يوجد توابع خاصة لفحصها:
...
تُستخدم طريقة الموازنة هذه غالبًا في توصيف JavaScript. عندما تحتاج خوارزمية لموازنة كون قيمتين متطابقتين تمامًا فإنها تستخدم `Object.is` (تُسَمَّى داخليًا القيمة ذاتها "SameValue").
...
لكن، يوجد لدينا في الواقع قيمًا بالوحدات، مثل `"100px"` أو `"12pt"` في CSS. في العديد من الدول أيضًا، يُلحَق رمز العملة بالقيمة، فمثلًا، لدينا `"19€"` ونريد استخراج قيمة عددية من ذلك.

هذا ما يقوم به التابعان `parseInt` و `parseFloat`؛ إذ يقرآن العدد من النص المعطى حتى يعجزا عن ذلك فتتوقف العملية. في حال وجود خطأ، يعيدان العدد المُجَمَّع. فيعيد التابع `parseInt` عددًا صحيحًا، بينما يعيد التابع `parseFloat` عددًا عشريًا:
...
```javascript
alert(parseInt('a123')); // NaN, الحرف الأول يوقف العملية
```
...
alert(parseInt('ff', 16)); // 255, وتعمل أيضًا بدون 0x
...
تُرجِع رقمًا عشوائيًا من 0 إلى 1 (لا يشمل 1).
...
تُرجِع  مرفوعًا للقوة المُعطاة:

```javascript
alert( Math.pow(2, 10) ); // 2 مرفوعًا للقوة 10 = 1024
```

يوجد المزيد من الدوال والثوابت في الكائن `Math`؛ بما في ذلك حساب المثلثات، والتي يمكنك العثور عليها في التوثيق الخاص بالكائن `Math`.
...
* `parseInt(str, base)` تحوِّل النص `str` إلى عدد صحيح بالنظام العددي المُعطى `base`؛ حيث .
* `num.toString(base)` تحوِّل العدد إلى نص بالنظام العددي المُعطى `base`.

**لتحويل القيم مثل `12pt` و `100px` إلى عدد:**

* استخدم `parseInt` أو `parseFloat` لتحويل سلس، والتي تقرأ العدد من نص وتُرجِع القيمة التي استطاعت قراءتها قبل حصول أي خطأ.
